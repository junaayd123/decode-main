...
//    private final Pose intake_from_gate = new Pose(56, 53, Math.toRadians(40));
 //   private final Pose intake_from_gate_rotate = new Pose(55, 54, Math.toRadians(0));
    ...
    private PathChain toHumanPlayerPath;
    private PathChain fromHumanPlayerPath;
...
// ===== HUMAN PLAYER PICKUP (original working version) =====
            ...
            case 132: // Brief reverse then drive back to farshotpose
                ...
                if (actionTimer.getElapsedTimeSeconds() > 0.2) {
                    ...
                    LL.set_angle_farredoptimized();
                    depo.setTargetVelocity(depo.ExcessRed - 50);
                    follower.followPath(fromHumanPlayerPath, true);
                    setPathState(133);
                }
                break;

            case 133: // Wait until back at farshotpose
                depo.updatePID();
                if (!follower.isBusy()) {
                    actionTimer.resetTimer();
                    setPathState(134);
            ...
            case 134: // Settle before final HP shot
                depo.updatePID();
                intake.setPower(1);
                if (actionTimer.getElapsedTimeSeconds() > SETTLE_TIME) {
                    intake.setPower(0);
            ...
            case 135: // Final HP shot done - kill everything
                if (actionState == 0) {
                    LL.allDown();
