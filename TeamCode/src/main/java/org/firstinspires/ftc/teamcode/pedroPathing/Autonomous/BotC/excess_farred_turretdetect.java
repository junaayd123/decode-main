 ...
 */
@Autonomous(name = "excess_farred_turretdetect", group = "Pedro")
public class excess_farred_turretdetect extends OpMode {
    ...
    private static final double TURRET_DETECT_DEGREES_2 = -16;
    ...
    private final Pose gateCollectDeepPose     = new Pose(73,  49,  Math.toRadians(85));  // actual collect position
    ...
    public void init() {
        ...
        telemetry.addLine("excess_farred_turretdetect initialized | preload → third → excess → detect-in-place → branch");
        telemetry.update();
    }
    ...
    public void loop() {
        ...
        if (pathState == 50 || pathState == 51 || pathState == 511) {
            telemetry.addData("Scan Step",       pathState == 511 ? "ROI1 (left)" : "ROI2 (right)");
            ...
            telemetry.addData("Saved ROI2",      String.format("%.2f%%", savedFirstRoiCombined));
            if (pathState == 511) {
                telemetry.addData("DECISION", ballCoverage.roi1CombinedPercent > savedFirstRoiCombined ? "GATE COLLECT" : "HUMAN PLAYER");
            }
        }
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            // ===== BLOB DETECTION & DECISION (two-step scan) =====
            case 50: // Enable processor, capture ROI2 (right) at first turret angle
                intake.setPower(0);
            ...
            case 51: // Wait for ROI2 reading, save it, turn turret further left for ROI1
                if (actionTimer.getElapsedTimeSeconds() >= DETECTION_WAIT) {
                    savedFirstRoiCombined = ballCoverage.roi2CombinedPercent;
            ...
            case 511: // Wait for ROI1 reading, then branch: ROI1 > saved ROI2 → gate, else → HP
                if (actionTimer.getElapsedTimeSeconds() >= DETECTION_WAIT) {
                    if (ballCoverage.roi1CombinedPercent > savedFirstRoiCombined) {
                        intake.setPower(-1);
                        actionTimer.resetTimer();
                        setPathState(52); // ROI1 more — collect from gate
                    } else {
                        setPathState(60); // ROI2 more — go to HP
                    }
                }
                break;
...
//                .setTimeoutConstraint(1.5)
    ...
    // Dual-ROI detection using YCrCb color thresholds with 5° rotation.
    ...
    static class BallCoverageProcessor implements VisionProcessor {
        ...
        public Object processFrame(Mat frame, long captureTimeNanos) {
            ...
            // 10. Hough circle detection on ROI1 (left) combined mask
