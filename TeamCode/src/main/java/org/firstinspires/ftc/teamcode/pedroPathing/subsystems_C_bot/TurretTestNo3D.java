...
@TeleOp(name = "Turret localization test", group = "tuning")
@Config
public class TurretTestNo3D extends LinearOpMode {
    ...
    public static Pose2d computeRobotPoseFromTag2D(
            double turretAngleRad,
            double camX, double camY, double camYawOffset,
            double camToTagX, double camToTagY, double camToTagZ,
            Pose2d tagFieldPose
    ) {
        // ---------------------------------------------------------
        // 1. Extract camera→tag translation (meters)
        // ---------------------------------------------------------
        double cx = camToTagX;   // left/right relative to camera
        double cy = camToTagY;   // up/down relative to camera (ignored for 2D)
        double cz = camToTagZ;   // forward/back relative to camera

        // ---------------------------------------------------------
        // 2. Compute ground-plane distance (tilt-safe)
        // ---------------------------------------------------------
        double groundDist = Math.sqrt(cx * cx + cz * cz);

        // ---------------------------------------------------------
        // 3. Compute yaw from camera to tag (tilt-safe)
        // ---------------------------------------------------------
        double yawToTag = Math.atan2(cx, cz);

        // ---------------------------------------------------------
        // 4. Compute camera heading in FIELD frame
        // ---------------------------------------------------------
        double cameraHeading = turretAngleRad + camYawOffset;

        // Absolute angle from camera to tag in FIELD frame
        double angleCameraToTag = cameraHeading + yawToTag;

        // ---------------------------------------------------------
        // 5. Compute camera FIELD position
        // ---------------------------------------------------------
        double cameraX = tagFieldPose.getX() - groundDist * Math.cos(angleCameraToTag);
        double cameraY = tagFieldPose.getY() - groundDist * Math.sin(angleCameraToTag);

        // ---------------------------------------------------------
        // 6. Compute camera offset in ROBOT frame (turret rotation)
        // ---------------------------------------------------------
        double cos = Math.cos(turretAngleRad);
        double sin = Math.sin(turretAngleRad);

        double camOffsetX = camX * cos - camY * sin;
        double camOffsetY = camX * sin + camY * cos;

        // ---------------------------------------------------------
        // 7. Convert camera→robot offset into FIELD frame
        // ---------------------------------------------------------
        double robotX = cameraX - camOffsetX;
        double robotY = cameraY - camOffsetY;

        // ---------------------------------------------------------
        // 8. Robot heading = camera heading
        // ---------------------------------------------------------
            ...
            if (result.getTimestamp() != lastTimestamp) {
                ...
                for (LLResultTypes.FiducialResult fr : fiducialResults) {
                    ...
                    if (fr.getFiducialId() == 20) {
                        ...
                        double camToTagX = camToTag.getPosition().x;
                        double camToTagY = camToTag.getPosition().y;
                        double camToTagZ = camToTag.getPosition().z;
                        Pose2d tagPose = new Pose2d(0, 0, new Rotation2d(0));
                        Pose2d robotFieldPose = computeRobotPoseFromTag2D( turretPoseRad, CAM_X, CAM_Y, CAM_YAW_OFFSET, camToTagX, camToTagY, camToTagZ, tagPose);
                        // Convert to inches for Pedro
                        Pose2d robotFieldPoseInches = new Pose2d( robotFieldPose.getX() * METERS_TO_INCHES, robotFieldPose.getY() * METERS_TO_INCHES, robotFieldPose.getRotation() );
                        telemetry.addData("Robot X (in)", robotFieldPoseInches.getX()); telemetry.addData("Robot Y (in)", robotFieldPoseInches.getY()); telemetry.addData("Robot Heading (deg)", Math.toDegrees(robotFieldPoseInches.getHeading()));
                    }
