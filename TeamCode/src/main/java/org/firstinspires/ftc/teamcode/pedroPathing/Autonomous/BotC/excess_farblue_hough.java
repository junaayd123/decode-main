 ...
 */
@Autonomous(name = "excess_farblue_hough", group = "Pedro")
public class excess_farblue_hough extends OpMode {
    ...
    private static final double TURRET_OFFSET = -5;
    ...
    public void init() {
        ...
        telemetry.addLine("excess_farblue_hough initialized | preload → third → excess → detect-in-place → hough-guided gate");
        telemetry.update();
    }
    ...
    public void loop() {
        ...
        if (pathState == 521 || pathState == 522 || pathState == 523) {
    ...
    //           → 40→41→50→51 (turret detect in place → branch)
    //           → 521→522→53→54→55→56 (gate collect → return → shoot)
    ...
    private void sampleBlueScanFrame() {
    ...
    private double computeGateYFromRawX(double rawX) {
        // Mirror the regression's field-Y output for blue.
    ...
    // Single-ROI detection using YCrCb thresholds.
    // Pipeline mirrors red behavior but flips horizontally first for blue.
    // Decision is Hough-based: circles found => gate collect, else HP fallback.
    static class BallCoverageProcessor implements VisionProcessor {
        ...
        private final Mat flippedMat  = new Mat();
        ...
        public Object processFrame(Mat frame, long captureTimeNanos) {
            ...
            // 2. Horizontal flip (blue-specific preprocessing)
            Core.flip(rgbMat, flippedMat, 1);

            // 3. Rotate
            Imgproc.warpAffine(flippedMat, rotatedMat, rotMatrix,
            ...
            // 6. Morphological cleanup — MORPH_OPEN → MORPH_CLOSE → dilate×2 → erode×1
            ...
            // 10. Hough circle detection on full-width ROI
